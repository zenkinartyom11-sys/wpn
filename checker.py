...
# РАБОЧИЙ ДОМЕН ДЛЯ ОБХОДА БЛОКИРОВОК
...
def parse_vless_link(link):
    ...
    try:
        ...
        service_name = query_params.get("serviceName", [""])[0] # Для gRPC
...
def modify_config(json_data, new_data):
    """Динамически перестраивает структуру outbounds с учетом gRPC / TCP / WS"""
    ...
    if vnext_list and len(vnext_list) > 0:
        ...
        # Настройка XTLS Flow для Reality TCP (для gRPC/WS flow удаляется)
    ...
    # 2. Перенастройка сетевого слоя (streamSettings)
    stream_settings = {
        "network": new_data["type"],
        "security": new_data["security"]
    }

    # Настройки шифрования (REALITY или TLS)
    if new_data["security"] == "reality":
        stream_settings["realitySettings"] = {
            "show": False,
            "fingerprint": "chrome",
            "serverName": BYPASS_DOMEN,
    ...
    # ИСПРАВЛЕНИЕ: Настройки транспорта (gRPC, WS или TCP) в зависимости от ссылки
...
def main():
    ...
    for line in res_keys.text.splitlines():
        ...
        if parsed_server:
            print(f"-> Найдена рабочая ссылка. Тип сети: {parsed_server['type']}, Безопасность: {parsed_server['security']}")
            break
    ...
    print("3. Перестраиваем структуру JSON под транспорт ссылки...")
    ...
    print("✅ JSON исправлен. Конфликт gRPC и TCP устранен.")
