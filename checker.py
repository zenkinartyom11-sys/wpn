...
def parse_vless_link(link):
    ...
    try:
        ...
        # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Если host в ссылке не указан, 
        # он ОБЯЗАН быть равен sni, иначе ws-прокси не пропустит трафик
        if sni and not host:
            host = sni
...
def modify_config(json_data, new_data):
    """Обновляет ваш JSON, сохраняя структуру vless/ws/tls и синхронизируя Host"""
    ...
    # Управление Host (headers) — теперь здесь гарантированно будет значение
...
def main():
    ...
    for line in res_keys.text.splitlines():
        ...
        if parsed_server:
            ...
            print(f"   SNI: {parsed_server['sni']} | Host: {parsed_server['host']}")
            break
    ...
    print("✅ Файл успешно изменен и синхронизирован.")
