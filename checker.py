...
def check_geoip_api(ip):
    try:
        # ИСПРАВЛЕНО: Добавлен корректный эндпоинт /json/
        response = requests.get(f"http://ip-api.com{ip}", timeout=2).json()
        # ИСПРАВЛЕНО: Теперь возвращает False для России (чтобы отсечь её при if not check_geoip_api)
        if response.get("countryCode") == "RU":
            return False
        return True
    except Exception:
        # Если API лежит, не блокируем сервер, даем шанс пройти TLS-проверку
        return True
...
def check_via_xray_core(link, xray_path, timeout=5):
    ...
    try:
        ...
        # ИСПРАВЛЕНО: Парсинг параметров Reality приведен к типам данных Xray (строки из списков)
        stream_settings = {
...
def main():
    ...
    # Ваше оригинальное окончание скрипта:
